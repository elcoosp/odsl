...
async fn applies_create_and_add_field_to_live_sqlite() {
    ...
    let db_path = dir.join(format!("odsl_test_{}.db", std::process::id()));
...
async fn applies_reference_as_foreign_key() {
    let dir = std::env::temp_dir();
    let db_path = dir.join(format!("odsl_test_fk_{}.db", std::process::id()));
    let _ = std::fs::remove_file(&db_path);
    let url = format!("sqlite:///{}?mode=rwc", db_path.display());
...
async fn history_tracker_records_and_detects_applied() {
    let dir = std::env::temp_dir();
    let db_path = dir.join(format!("odsl_hist_{}.db", std::process::id()));
    let _ = std::fs::remove_file(&db_path);
    let url = format!("sqlite:///{}?mode=rwc", db_path.display());
    ...
    let schema = run_sqlite(&db_path, ".schema _odsl_migrations");
    assert!(schema.contains("_odsl_migrations"), "history table missing");
