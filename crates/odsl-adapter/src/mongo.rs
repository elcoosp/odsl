...
/// Map an ODSL scalar keyword to its BSON type name.
...
/// string field. Values are stored verbatim (the ODSL validator has already
/// confirmed the columns exist on the model).
fn seed_docs(seed: &odsl_core::ast::LockSeed) -> Vec<Document> {
