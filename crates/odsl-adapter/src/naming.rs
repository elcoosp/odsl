...
//! [`odsl_core::naming`] so the migrations adapter agrees with the code
...
/// renderer emits (so migrations target the collections `odsl build` creates).
