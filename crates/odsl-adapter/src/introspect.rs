//! Reverse-engineering: introspect a live SQL database and emit ODSL source.
//!
//! `odsl pull --db-url <url>` calls [`introspect_to_odsl`] to connect to the
...
//! ODSL schema. The result is a starting point for a human to refine — it is
//! intentionally lossy about things ODSL cannot express (composite non-unique
...
/// Introspect `db_url` and return ODSL source text describing the schema.
pub async fn introspect_to_odsl(db_url: &str) -> Result<String, String> {
    ...
    Ok(render_odsl(&tables, &columns, &indexes))
}
...
/// List user tables for the dialect (skips the ODSL history tracker and system
/// catalogs).
    ...
    let sql = match dialect {
        ...
        Dialect::Postgres => {
            "SELECT table_name FROM information_schema.tables \
             ...
             AND table_name <> '_odsl_migrations' ORDER BY table_name"
        }
        Dialect::Mysql => {
            "SELECT table_name FROM information_schema.tables \
...
/// Map a native SQL type string to an ODSL scalar keyword (best effort).
...
/// Render the collected catalog into ODSL source text.
fn render_odsl(tables: &[String], columns: &[ColumnInfo], indexes: &[IndexInfo]) -> String {
    let mut out = String::new();
    writeln!(
        out,
        "# ODSL schema (reverse-engineered from a live database)."
    )
    .ok();
    ...
    for table in tables {
        ...
        // No trailing comment here: the ODSL parser does not accept inline
        // comments on intent lines, so the emitted schema must round-trip
        // cleanly through `odsl build`.
...
/// Convert a table name to a singular PascalCase ODSL model name.
...
mod tests {
    ...
    #[test]
    fn render_odsl_preserves_tz_on_timestamptz() {
        ...
        let out = render_odsl(&tables, &columns, &[]);
        assert!(
    ...
    #[test]
    fn render_odsl_no_tz_for_plain_timestamp() {
        let tables = vec!["users".to_string()];
        let columns = vec![ColumnInfo {
            table: "users".to_string(),
            name: "created_at".to_string(),
