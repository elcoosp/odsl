//! Live database adapters for the ODSL compiler.
//!
...
use odsl_migrator::MigrationPlan;
...
pub trait SchemaAdapter: Send + Sync {
    ...
    /// Create the idempotency history table (`_odsl_migrations`) if absent.
    ...
    /// Drop every user table/collection (and the ODSL history tracker) so a
...
/// host). Falls back to `odsl` when absent.
...
impl SqlAdapter {
    ...
    const HISTORY_TABLE: &'static str = "_odsl_migrations";
    ...
    /// List user tables (excludes the ODSL history tracker and system
    ...
    async fn list_user_tables(&self) -> Result<Vec<String>, AdapterError> {
        let sql = match self.dialect {
            sql::SqlDialect::Sqlite => {
                "SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' AND name <> '_odsl_migrations'"
            }
            sql::SqlDialect::Postgres => {
                "SELECT table_name FROM information_schema.tables \
                 WHERE table_schema = 'public' AND table_type = 'BASE TABLE' \
                 AND table_name <> '_odsl_migrations'"
            }
...
mod tests {
    ...
    fn mongo_db_name_parsing() {
        ...
        assert_eq!(db_name_from_url("mongodb://localhost:27017/"), "odsl");
        assert_eq!(
