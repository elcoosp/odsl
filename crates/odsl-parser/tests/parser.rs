//! Integration tests for the ODSL parser, exercising the full parse pipeline
//! and the spec's BDD acceptance scenarios where applicable.

use odsl_core::ast::Ast;
use odsl_core::errors::{CompileErrorKind, OdslError};
use odsl_core::lockfile::Lockfile;
use odsl_core::types::{FieldType, FkAction, Intent, ScalarType};
use odsl_parser::infer;
use odsl_parser::parse;
use proptest::prelude::*;

fn find_model<'a>(ast: &'a Ast, name: &str) -> &'a odsl_core::ast::Model {
    let idx = ast.model_by_name(name).expect("model should exist");
    &ast.models[idx]
}

fn find_field<'a>(model: &'a odsl_core::ast::Model, name: &str) -> &'a odsl_core::ast::Field {
...
fn formulates_precision_roundtrip() {
    ...
    use odsl_core::formatter::format_ast;
    let src = "Product\n  id uuid -pk\n  price numeric -precision 18,4\n";
    let ast = parse(src).unwrap();
...
fn rejects_unknown_intent_flag() {
    ...
    assert!(matches!(err, OdslError::Parse(_)));
}
...
fn cyclic_dependency_detected_by_validator() {
    ...
    let ast = parse(src).unwrap();
    let err =
        odsl_core::Validator::validate(&ast, Some(odsl_core::Target::SeaOrmSqlite)).unwrap_err();
    match err {
        OdslError::Compile { kind, .. } => assert_eq!(
            kind,
...
fn target_incompatibility_detected() {
    ...
    let err =
        odsl_core::Validator::validate(&ast, Some(odsl_core::Target::SeaOrmSqlite)).unwrap_err();
    match err {
        OdslError::Compile { kind, .. } => assert!(matches!(
            kind,
...
fn records_use_declarations_single_file() {
    let src = "use user\nuse billing::invoice\nOrder\n  id uuid -pk\n  user User.id\n  invoice Invoice.id\n";
    let file = odsl_parser::parse_file(src).unwrap();
    assert_eq!(
        file.uses,
...
fn resolves_and_merges_modules_via_use() {
    let dir = std::env::temp_dir().join(format!("odsl-use-{}", std::process::id()));
    let _ = std::fs::create_dir_all(&dir);
    let user_mod = dir.join("user.odsl");
    let billing = dir.join("billing");
    std::fs::create_dir_all(&billing).unwrap();
    let invoice = billing.join("invoice.odsl");

    std::fs::write(&user_mod, "User\n  id uuid -pk\n  email string -uniq\n").unwrap();
    std::fs::write(&invoice, "Invoice\n  id uuid -pk\n  total int\n").unwrap();
    std::fs::write(
        dir.join("schema.odsl"),
        "use user\nuse billing::invoice\nOrder\n  id uuid -pk\n  user User.id\n  invoice Invoice.id\n  total int\n",
    )
    .unwrap();

    let project = odsl_parser::parse_project(&dir.join("schema.odsl")).unwrap();
...
fn use_detects_duplicate_model_across_files() {
    let dir = std::env::temp_dir().join(format!("odsl-use-dup-{}", std::process::id()));
    let _ = std::fs::create_dir_all(&dir);
    std::fs::write(dir.join("a.odsl"), "User\n  id uuid -pk\n").unwrap();
    std::fs::write(dir.join("b.odsl"), "User\n  id uuid -pk\n").unwrap();
    std::fs::write(dir.join("schema.odsl"), "use a\nuse b\n").unwrap();
    let err = odsl_parser::parse_project(&dir.join("schema.odsl")).unwrap_err();
    assert!(matches!(err, OdslError::Parse(_)));
    let _ = std::fs::remove_dir_all(&dir);
}
...
fn use_cycle_is_safe() {
    ...
    let dir = std::env::temp_dir().join(format!("odsl-use-cycle-{}", std::process::id()));
    let _ = std::fs::create_dir_all(&dir);
    std::fs::write(dir.join("a.odsl"), "use b\nA\n  id uuid -pk\n").unwrap();
    std::fs::write(dir.join("b.odsl"), "use a\nB\n  id uuid -pk\n").unwrap();
    std::fs::write(dir.join("schema.odsl"), "use a\n").unwrap();
    let project = odsl_parser::parse_project(&dir.join("schema.odsl")).unwrap();
...
";
    ...
    assert_eq!(
        author.ty,
        FieldType::Ref(odsl_core::types::Reference {
            model: "User".into(),
            field: "id".into(),
        })
    );
...
fn config_suppresses_missing_timestamps_lint() {
    use odsl_core::lint::{LintConfig, LintRule, Linter};
    let src = "config\n  audit created_at,updated_at\n\nUser\n  id uuid -pk\n";
    let ast = parse(src).unwrap();
...
fn view_with_equals_but_empty_query_is_rejected_at_validation() {
    ...
    let err = odsl_core::validator::Validator::validate(&ast, None).unwrap_err();
