//! The ODSL parser: pest token stream -> validated [`Ast`].
//!
...
use odsl_core::errors::{OdslError, ParseError, Span};
use odsl_core::types::{FieldType, FkAction, Intent, Reference, ScalarType};
use pest::Parser;
...
/// Parse ODSL source into a resolved [`Ast`] (inference applied).
pub fn parse_and_resolve(src: &str) -> Result<Ast, OdslError> {
    parse(src)
}
...
/// Parse a single ODSL source file into an [`Ast`] (inference applied).
/// `use` declarations are recorded but not resolved (see [`parse_project`]).
pub fn parse_file(src: &str) -> Result<FileAst, OdslError> {
    let pairs = grammar::OdslParser::parse(Rule::file, src)
        .map_err(|e| OdslError::Parse(ParseError::new(format!("parse error: {e}"))))?;
            ...
            && let Some(RawToken::TypeDecl { name, rhs }) = pl.tokens.first()
        {
            let ct =
                custom_type_from_tokens(name, rhs, src, pl.byte_start, pl.byte_end, pl.line_no)?
                    .ok_or_else(|| {
                        OdslError::Parse(ParseError::new(format!(
        ...
        if pl.indent == 0 {
            ...
            let m =
                match current_model.as_mut() {
                    Some(m) => m,
                    None => return Err(OdslError::Parse(
                        ParseError::new(
...
/// Parse ODSL source into a resolved [`Ast`]. `use` declarations are parsed
/// but not resolved (single-file view). For multi-file module resolution use
/// [`parse_project`].
pub fn parse(src: &str) -> Result<Ast, OdslError> {
...
/// Resolve a project rooted at `root` (an `.odsl` file). Recursively follows
/// `use` declarations, merging each imported file's models into a single AST.
///
/// Resolution rules:
/// * `use a::b::c` maps to `<root_dir>/a/b/c.odsl` (also tries `<root_dir>/a/b/c`
///   without extension and `<root_dir>/a/b/c/mod.odsl`).
/// * A model name may not collide across files (returns an error).
/// * Cycles are detected via a visited set.
pub fn parse_project(root: &std::path::Path) -> Result<Project, OdslError> {
    let mut ast = Ast::new();
...
fn resolve_file(
    ...
    for (_, model) in file_ast.ast.models() {
        if ast.model_by_name(&model.name).is_some() {
            return Err(OdslError::Parse(ParseError::new(format!(
...
/// Map `a::b::c` to a concrete `.odsl` path, trying a few conventions.
fn resolve_use_path(dir: &std::path::Path, use_path: &str) -> std::path::PathBuf {
    let rel = use_path.replace("::", "/");
    let base = dir.join(&rel);
    // Try, in order: <rel>.odsl, <rel>/mod.odsl, <rel> (bare).
    let candidates = [
        format!("{}.odsl", base.to_string_lossy()),
        format!("{}/mod.odsl", base.to_string_lossy()),
    ...
    // Default to the `.odsl` form even if it doesn't exist (error surfaces later).
    std::path::PathBuf::from(format!("{}.odsl", base.to_string_lossy()))
}
...
fn parse_view_decl(tokens: &[RawToken], line_no: usize, raw: &str) -> Result<View, OdslError> {
...
fn parse_seed_row(raw: &str, line_no: usize) -> Result<SeedRow, OdslError> {
...
fn add_field_from_tokens(
    ...
    byte_end: usize,
    src: &str,
) -> Result<(Option<String>, Option<String>), OdslError> {
...
fn custom_type_from_tokens(
    ...
    byte_end: usize,
    line_no: usize,
) -> Result<Option<CustomType>, OdslError> {
...
fn collect_lines(pairs: Pairs<Rule>, src: &str) -> Result<Vec<ParsedLine>, OdslError> {
...
fn parse_line(pair: Pair<Rule>, src: &str) -> Result<ParsedLine, OdslError> {
                        ...
                        Rule::token => {
                            let t =
                                b.into_inner().next().ok_or_else(|| {
                                    OdslError::Parse(ParseError::new("empty token").with_span(
...
mod enum_tests {
    ...
    fn parses_seed_rows() {
        let src = "User\n  id uuid -pk\n  email string\n\
seed User\n  id=00000000-0000-0000-0000-000000000001 email=\"root@odsl.dev\"\n  id=00000000-0000-0000-0000-000000000002 email=\"user@odsl.dev\"\n";
        let ast = parse(src).unwrap();
        ...
        assert_eq!(
            ...
            vec![
                ...
                ("email".to_string(), "root@odsl.dev".to_string()),
            ]
        );
        // Column order is preserved within a row; the second row is distinct.
        let r1 = &seed.rows[1];
        assert_eq!(r1.columns[1].1, "user@odsl.dev");
    }
...
seed User\n  id=1 brokenrow\n";
    ...
    fn parse_project_merges_modules() {
        use std::io::Write;
        let dir = std::env::temp_dir().join(format!("odsl_proj_{}", std::process::id()));
...
seed Invoice\n  id=00000000-0000-0000-0000-0000000000aa total=42\n";
        ...
        let entry_path = dir.join("main.odsl");
        let invoice_path = mod_dir.join("invoice.odsl");
        {
    ...
    fn parse_project_detects_duplicate_model_across_files() {
        use std::io::Write;
        let dir = std::env::temp_dir().join(format!("odsl_dup_{}", std::process::id()));
        let _ = std::fs::create_dir_all(&dir);
        let mod_dir = dir.join("billing");
        let _ = std::fs::create_dir_all(&mod_dir);
