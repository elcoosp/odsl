//! Error and diagnostic types for the ODSL compiler.
//!
...
/// Source span within a `.odsl` file (0-indexed byte offsets).
...
pub struct ParseError {
    ...
    /// The full `.odsl` source, used to render the label.
...
impl Diagnostic for OdslError {
    fn labels(&self) -> Option<Box<dyn Iterator<Item = LabeledSpan> + '_>> {
        match self {
            OdslError::Parse(p) => p.labels(),
            OdslError::Compile {
                span: Some(s),
                kind,
                ..
    ...
    fn source_code(&self) -> Option<&dyn SourceCode> {
        match self {
            OdslError::Parse(p) => p.source_code(),
            OdslError::Compile { src: Some(s), .. } => Some(s as &dyn SourceCode),
            _ => None,
        }
    }
}
...
mod tests {
    ...
    fn io_from_converts_with_chain() {
        ...
        let e: OdslError = io_err.into();
        assert!(matches!(e, OdslError::Io(_)));
    ...
    fn json_from_converts() {
        ...
        let e: OdslError = json_err.into();
        assert!(matches!(e, OdslError::Json(_)));
    }
}
