...
use crate::errors::{CompileErrorKind, OdslError};
...
impl Validator {
    ...
    pub fn validate(ast: &Ast, target: Option<Target>) -> Result<(), OdslError> {
    ...
    fn resolve_references(ast: &Ast) -> Result<(), OdslError> {
        for (_midx, model) in ast.models() {
            for (_fidx, field) in model.fields() {
                ...
                {
                    return Err(OdslError::compile(CompileErrorKind::UnresolvedReference {
    ...
    fn check_keys(ast: &Ast) -> Result<(), OdslError> {
        for (_midx, model) in ast.models() {
            ...
            if !model.primary_key.is_empty() {
                ...
                if model.fields().any(|(_, f)| f.has(crate::types::Intent::Pk)) {
                    return Err(OdslError::compile(CompileErrorKind::InvalidKey {
            ...
            let keys = model
                ...
                .count();
            if keys != 1 {
                return Err(OdslError::compile(CompileErrorKind::MissingKey {
                    model: model.name.clone(),
                }));
            }
        }
    ...
    fn check_intent_compat(ast: &Ast) -> Result<(), OdslError> {
        for (_midx, model) in ast.models() {
            for (_fidx, field) in model.fields() {
                for intent in &field.intents {
                    if !is_intent_compatible(*intent, &field.ty) {
                        return Err(OdslError::compile(CompileErrorKind::TypeMismatch {
    ...
    fn check_model_indexes(ast: &Ast) -> Result<(), OdslError> {
        for (_midx, model) in ast.models() {
    ...
    fn check_m2m_targets(ast: &Ast) -> Result<(), OdslError> {
        for (_midx, model) in ast.models() {
            for (_fidx, field) in model.fields() {
    ...
    fn check_views(ast: &Ast) -> Result<(), OdslError> {
        use crate::types::ScalarType;
        let mut seen = std::collections::HashSet::new();
        for v in ast.views() {
            if v.name.is_empty() {
                return Err(OdslError::compile(CompileErrorKind::ViewError {
                    view: v.name.clone(),
    ...
    fn check_seeds(ast: &Ast) -> Result<(), OdslError> {
        let mut seen = std::collections::HashSet::new();
        for s in ast.seeds() {
            if s.model.is_empty() {
                return Err(OdslError::compile(CompileErrorKind::SeedError {
                    model: s.model.clone(),
    ...
    fn check_target_compat(ast: &Ast, target: Target) -> Result<(), OdslError> {
        for (_midx, model) in ast.models() {
            for (_fidx, field) in model.fields() {
                for intent in &field.intents {
                    if !target_supports(target, *intent, &field.ty) {
                        return Err(OdslError::compile(
    ...
    fn prevent_cycles(ast: &Ast) -> Result<(), OdslError> {
        ...
        for (_midx, model) in ast.models() {
            ...
            {
                return Err(OdslError::compile(CompileErrorKind::CyclicDependency {
                    models: cycle,
                }));
            }
        }
