...
use crate::errors::{OdslError, ParseError};
...
impl SchemaConfig {
    ...
    /// error.
    pub fn merge_from(&mut self, other: &SchemaConfig) -> Result<(), OdslError> {
        if let Some(other_val) = &other.default_type {
            match &self.default_type {
                Some(existing) if existing != other_val => {
                    return Err(OdslError::Parse(ParseError::new(
...
impl Ast {
    ...
    /// Tolerant structural comparison used by `odsl migrate test` to assert a
    ...
    /// because database round-trips normalize them (e.g. ODSL `int` is stored
    ...
    /// (e.g. an `_odsl_migrations` tracker); only `expected`'s requirements are
