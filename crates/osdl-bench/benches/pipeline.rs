//! Criterion benchmark for the OSDL compiler pipeline (REQ-NFR-PERF-001).
//!
...
//! `cargo bench -p osdl-bench`. The SLO assertion lives in `tests/slo.rs` so it
//! runs under `cargo test` (default harness) and fails CI on regression.

use criterion::{Criterion, criterion_group, criterion_main};
use osdl_bench::{build_schema, run_pipeline};
