//! ODSL Model Context Protocol (MCP) server.
//!
//! Wraps `odsl-core` so AI agents (Claude, Cursor, Copilot, …) can read,
//! validate, format and transpile an ODSL schema over the Model Context
...
//!
//! Exposed tools:
//! * `read_schema`     — parse a `.odsl` file and return a structured model list.
//! * `validate_schema` — parse + validate, returning structured diagnostics.
//! * `format_schema`   — canonicalise an `.odsl` file in place (or return result).
...
//!
//! Every tool takes a `path` argument (an `.odsl` file) and returns JSON the
...
/// and writing JSON-RPC responses to stdout. This is the `odsl mcp` entry point.
    ...
    match method {
        "initialize" => json!({
            ...
            "result": {
                ...
                "capabilities": { "tools": {} },
                "serverInfo": { "name": "odsl", "version": env!("CARGO_PKG_VERSION") }
            }
        }),
...
fn tools_list() -> Vec<Value> {
    vec![
        tool(
            "read_schema",
            "Parse an ODSL schema file and return its models and fields as structured JSON.",
            json!({
                "type": "object",
                "properties": {
                    "path": { "type": "string", "description": "Path to the .odsl file" }
                },
                "required": ["path"]
            }),
        ),
        tool(
            "validate_schema",
            "Parse and validate an ODSL schema, returning structured diagnostics an agent can fix.",
            json!({
                "type": "object",
                "properties": {
                    "path": { "type": "string", "description": "Path to the .odsl file" },
        ...
        tool(
            "format_schema",
            "Deterministically reformat an ODSL file in place (and return the canonical text).",
            json!({
                "type": "object",
                "properties": {
                    "path": { "type": "string", "description": "Path to the .odsl file" }
        ...
        tool(
            "build",
            "Transpile an ODSL schema to a target and return the generated file map.",
            json!({
                "type": "object",
                "properties": {
                    "path": { "type": "string", "description": "Path to the .odsl file" },
...
fn tool_validate_schema(args: &Value) -> Value {
    ...
    match odsl_core::Validator::validate(&ast, Some(parse_target(args))) {
...
fn tool_format_schema(args: &Value) -> Value {
    ...
    if let Err(e) = odsl_core::Validator::validate(&ast, Some(Target::SeaOrmSqlite)) {
        return json!({ "error": format!("validate: {e}") });
    }
    let formatted = odsl_core::formatter::format_ast(&ast);
...
fn tool_build(args: &Value) -> Value {
    ...
    if let Err(e) = odsl_core::Validator::validate(&ast, Some(target)) {
        return json!({ "error": format!("validate: {e}") });
    }
...
fn tool_migrate_preview(args: &Value) -> Value {
    ...
    let target_lock = odsl_core::lockfile::Lockfile::from_ast(&ast);
    ...
    let plan = match odsl_migrator::plan_migration(&current, &ast) {
        Ok(p) => p,
    ...
    let sql = match target {
        Target::SeaOrmSqlite | Target::SeaOrmPostgres | Target::SeaOrmMysql => {
            let dialect = match target {
                Target::SeaOrmSqlite => odsl_adapter::sql::SqlDialect::Sqlite,
                Target::SeaOrmPostgres => odsl_adapter::sql::SqlDialect::Postgres,
                Target::SeaOrmMysql => odsl_adapter::sql::SqlDialect::Mysql,
                _ => unreachable!(),
            };
            let up: Vec<String> = plan
                ...
                .iter()
                .flat_map(|op| {
                    odsl_adapter::sql::op_to_sql(dialect, op, &target_lock, Some(&current))
                        .unwrap_or_else(|e| vec![format!("-- ERROR: {e}")])
                })
                .collect();
            let down = odsl_adapter::migrate::render_down_sql(
                dialect,
                &plan,
                &target_lock,
                Some(&current),
...
fn render_target(
    target: Target,
    ast: &odsl_core::ast::Ast,
) -> Result<Vec<(String, String)>, odsl_core::errors::OdslError> {
    match target {
        Target::SeaOrmSqlite | Target::SeaOrmPostgres | Target::SeaOrmMysql => {
            odsl_codegen_seaorm::SeaOrmRenderer::new(target).render(ast)
        }
        Target::Mongo => odsl_codegen_mongo::MongoRenderer::new(target).render(ast),
        Target::TypeScript => odsl_codegen_typescript::TypeScriptRenderer::new(target).render(ast),
        Target::GraphQl => odsl_codegen_graphql::GraphQLRenderer::new(target).render(ast),
        Target::OpenApi => odsl_codegen_openapi::OpenApiRenderer::new(target).render(ast),
        Target::JsonSchema => odsl_codegen_jsonschema::JsonSchemaRenderer::new(target).render(ast),
        Target::Zod => odsl_codegen_ts_validators::TsValidatorRenderer::new(
            target,
            odsl_codegen_ts_validators::ValidatorFlavor::Zod,
        )
        .render(ast),
        Target::Valibot => odsl_codegen_ts_validators::TsValidatorRenderer::new(
            target,
            odsl_codegen_ts_validators::ValidatorFlavor::Valibot,
        )
        .render(ast),
        Target::TypeBox => odsl_codegen_ts_validators::TsValidatorRenderer::new(
            target,
            odsl_codegen_ts_validators::ValidatorFlavor::TypeBox,
        )
        .render(ast),
        Target::Trpc => odsl_codegen_trpc::TrpcRenderer::new(target).render(ast),
    }
}
...
mod tests {
    ...
    fn initialize_returns_server_info() {
        ...
        assert_eq!(resp["result"]["serverInfo"]["name"], "odsl");
    }
    ...
    fn read_schema_returns_models() {
        ...
        let p = dir.join("mcp_read_test.odsl");
        std::fs::write(&p, "User\n  id uuid -pk\n  email string -uniq\n").unwrap();
    ...
    fn validate_schema_reports_errors() {
        let dir = std::env::temp_dir();
        let p = dir.join("mcp_validate_test.odsl");
        std::fs::write(&p, "Post\n  id uuid -pk\n  author User.id\n").unwrap();
        let args = json!({ "path": p.to_string_lossy() });
    ...
    fn lint_returns_findings_for_dirty_schema() {
        let dir = std::env::temp_dir();
        let p = dir.join("mcp_lint_test.odsl");
    ...
    fn migrate_preview_plans_create_models() {
        let dir = std::env::temp_dir();
        let p = dir.join("mcp_preview_test.odsl");
        std::fs::write(&p, "User\n  id uuid -pk\n  email string -uniq\n").unwrap();
