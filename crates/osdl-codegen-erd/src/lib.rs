...
pub fn render(ast: &Ast, format: ErdFormat) -> Result<(String, String), String> {
    ...
    match format {
        ErdFormat::Mermaid => {
            let body = render_mermaid(&models);
            Ok(("schema.erd.md".to_string(), body))
        }
        ErdFormat::Dbml => {
            let body = render_dbml(&models);
...
fn render_mermaid(models: &[LockModel]) -> String {
...
fn render_dbml(models: &[LockModel]) -> String {
    let mut out = String::new();
    out.push_str("// Generated by `osdl erd --format dbml`. Do not edit by hand.\n");
