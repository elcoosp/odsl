...
//!
//! Generates, per OSDL model, a standalone JSON Schema document. A combined
    ...
    fn render(&self, ast: &Ast) -> Result<Vec<(String, String)>, OsdlError> {
        ...
        for model in &models {
            ...
            let text = serde_json::to_string_pretty(&schema).map_err(|e| {
                OsdlError::Io(std::io::Error::other(format!("serialize json-schema: {e}")))
            })?;
...
/// Map an OSDL type keyword (or `Model.field` reference) to a JSON Schema
/// fragment. References are stored as their (UUID/integer) id and annotated
/// with `x-osdl-ref` so the relationship survives.
fn json_type_for_ty(ty: &str) -> Value {
    if ty.contains('.') {
        return json!({ "type": "string", "x-osdl-ref": ty });
    }
    match ty {
        ...
        other => json!({ "type": "string", "x-osdl-unknown-type": other }),
    }
}
...
fn build_model_schema(model: &LockModel, ast: &Ast) -> Value {
    ...
    for f in &model.fields {
        ...
        if !f.polymorphic_targets.is_empty() {
            ...
            props.insert(
                format!("{base}_type"),
                json!({ "type": "string", "x-osdl-polymorphic": f.polymorphic_targets }),
            );
            props.insert(
                format!("{base}_id"),
        ...
        if let Some(obj) = schema.as_object_mut() {
            if has_intent(f, "-uniq") {
                obj.insert("x-osdl-unique".into(), json!(true));
            }
            if let Some(expr) = &f.check_expr {
                obj.insert("x-osdl-check".into(), json!(expr));
            }
...
fn build_combined(models: &[LockModel], ast: &Ast) -> Value {
    ...
    json!({
        "$schema": DRAFT,
        "title": "OSDL-generated JSON Schemas",
        "$id": "https://osdl.dev/schema.json",
        "description": "Generated by `osdl build --target json-schema`. Do not edit by hand.",
...
mod tests {
    ...
    fn render(src: &str) -> Vec<(String, String)> {
        ...
        osdl_core::Validator::validate(&ast, Some(Target::JsonSchema)).expect("validate");
    ...
    fn maps_types_and_required_and_nullable() {
        ...
        assert_eq!(props["email"]["x-osdl-unique"], json!(true));
    }
    ...
    fn expands_m2m_junctions() {
        ...
        assert_eq!(props["user_id"]["x-osdl-ref"], json!("User.id"));
    }
}
