//! ODSL Language Server.
//!
//! A real LSP implementation built on `async-lsp` (0.2, omni-trait API). It
//! parses and validates ODSL documents whenever they are opened or changed and
...
//! `odsl_parser::parse` then `odsl_core::Validator::validate`. This guarantees
//! that editor squiggles match `odsl build` / `odsl migrate` exactly.
...
use odsl_core::validator::Validator;
...
impl Backend {
    ...
    fn referenced_model(field: &odsl_core::Field) -> Option<String> {
        match &field.ty {
            odsl_core::FieldType::Ref(r) => Some(r.model.clone()),
            _ => None,
        }
    }
    ...
    fn hover_for_field(field: &odsl_core::Field) -> String {
...
fn rust_type_name(field: &odsl_core::Field) -> String {
    if field.has(odsl_core::Intent::Virtual) {
        return "⚠ virtual (not stored)".to_string();
    }
    let base = match &field.ty {
        odsl_core::FieldType::Scalar(s) => match s {
            odsl_core::ScalarType::String => "String",
            odsl_core::ScalarType::Int => "i32",
            odsl_core::ScalarType::BigInt => "i64",
            odsl_core::ScalarType::Float => "f64",
            odsl_core::ScalarType::Bool => "bool",
            odsl_core::ScalarType::DateTime => "chrono::DateTime<Utc>",
            odsl_core::ScalarType::Date => "chrono::NaiveDate",
            odsl_core::ScalarType::Uuid => "Uuid",
            odsl_core::ScalarType::Json => "Json",
            odsl_core::ScalarType::Binary => "Vec<u8>",
            odsl_core::ScalarType::Decimal => "Decimal",
        }
        .to_string(),
        odsl_core::FieldType::Ref(r) => format!("{}", r),
        odsl_core::FieldType::InferredRef(s) => s.clone(),
    };
    if field.has(odsl_core::Intent::Null) {
        format!("Option<{base}>")
    } else {
        base
    }
...
fn sql_column_definition(field: &odsl_core::Field) -> String {
    let col = field.name.clone();
    let ty = match &field.ty {
        odsl_core::FieldType::Scalar(s) => sql_type_for(*s).to_string(),
        odsl_core::FieldType::Ref(r) => format!("UUID REFERENCES \"{}\"(\"id\")", r.model),
        odsl_core::FieldType::InferredRef(s) => s.clone(),
    };
    let mut def = format!("\"{col}\" {ty}");
    if field.has(odsl_core::Intent::Pk) {
        def.push_str(" PRIMARY KEY");
    }
    if !field.has(odsl_core::Intent::Null) && !field.has(odsl_core::Intent::Pk) {
        def.push_str(" NOT NULL");
    }
    if field.has(odsl_core::Intent::Uniq) && !field.has(odsl_core::Intent::Pk) {
        def.push_str(" UNIQUE");
    }
...
fn sql_type_for(s: odsl_core::ScalarType) -> &'static str {
    match s {
        odsl_core::ScalarType::String => "TEXT",
        odsl_core::ScalarType::Int => "INTEGER",
        odsl_core::ScalarType::BigInt => "BIGINT",
        odsl_core::ScalarType::Float => "DOUBLE",
        odsl_core::ScalarType::Bool => "BOOLEAN",
        odsl_core::ScalarType::DateTime => "TIMESTAMPTZ",
        odsl_core::ScalarType::Date => "DATE",
        odsl_core::ScalarType::Uuid => "UUID",
        odsl_core::ScalarType::Json => "JSONB",
        odsl_core::ScalarType::Binary => "BYTEA",
        odsl_core::ScalarType::Decimal => "NUMERIC",
    }
}
...
fn field_at_line(ast: &odsl_core::Ast, line: usize) -> Option<odsl_core::Field> {
...
mod tests {
    ...
    fn diagnostics_flag_parse_errors() {
        let diags = Backend::diagnostics_for("this is not valid odsl @@@");
        assert!(
            !diags.is_empty(),
